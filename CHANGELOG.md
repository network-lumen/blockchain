...
- Slashing: new `MsgUpdateSlashingDowntimeParams` in `x/tokenomics` lets the chain authority safely tune downtime penalties with built-in bounds:
  - `slash_fraction_downtime` must be between 0 and 0.05 (0–5%).
  - `downtime_jail_duration` must be between 60s and 48h.
  - Double-sign slashing parameters remain immutable and are not exposed by this message.

...
Example:
	lumend multi-node --v 4 --output-dir ./.testnets --validators-stake-amount 1000000,200000,300000,400000 --list-ports 47222,50434,52851,44210
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if args.minGasPrices != "" && !strings.EqualFold(args.minGasPrices, "0ulmn") {
				return fmt.Errorf("gasless chain: minimum-gas-prices must be 0ulmn or unset (got %s)", args.minGasPrices)
			}
...
func addTestnetFlagsToCmd(cmd *cobra.Command) {
	...
	cmd.Flags().String(server.FlagMinGasPrices, "0ulmn", "Gasless chain: this flag must remain 0ulmn or unset")
...
func initTestnetFiles(
	...
	if args.minGasPrices == "" {
		args.minGasPrices = "0ulmn"
	}

	appConfig := srvconfig.DefaultConfig()
	appConfig.MinGasPrices = args.minGasPrices
	appConfig.API.Enable = false
	appConfig.BaseConfig.MinGasPrices = args.minGasPrices

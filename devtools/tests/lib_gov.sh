...
gov_submit_single_msg_proposal() {
  local msg_json="$1"
  local title="${2:-Governance proposal}"
  local summary="${3:-autogenerated}"
  local deposit="${4:-10000000ulmn}"

  local proposal_file
  proposal_file=$(mktemp -t gov-single-msg-XXXXXX)
  jq -n --arg title "$title" --arg summary "$summary" --arg metadata "" --argjson msg "$msg_json" \
    '{messages: [$msg], title: $title, summary: $summary, metadata: $metadata}' >"$proposal_file"

...
for t in "${targets[@]}"; do
  ...
  mkdir -p "$outdir"
  echo "-- $t"
  (cd "$DIR" && CGO_ENABLED=0 GOOS=$GOOS GOARCH=$GOARCH go build -trimpath -ldflags "$LDFLAGS" -o "$outdir/$bin_name" ./cmd/lumend)
done

echo "==> Generating SHA256SUMS"
(
  cd "$DIST/$VERSION"
  find . -type f \( -name 'lumend' -o -name 'lumend.exe' \) -exec sha256sum {} \; | sed 's|\./||' > SHA256SUMS
)

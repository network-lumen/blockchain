...
const (
	SimAppChainID = "lumen-simapp"
)

var FlagEnableStreamingValue bool
var FlagEnableSimulation bool

func init() {
	simcli.GetSimulatorFlags()
	flag.BoolVar(&FlagEnableStreamingValue, "EnableStreaming", false, "Enable streaming service")
	flag.BoolVar(&FlagEnableSimulation, "EnableSimulation", false, "enable full application simulations")
}
...
func BenchmarkSimulation(b *testing.B) {
	...
	if !FlagEnableSimulation {
		b.Skip("skipping simulation benchmark; pass -EnableSimulation to run")
	}

	config := simcli.NewConfigFromFlags()
	config.ChainID = SimAppChainID

	db, dir, logger, skip, err := simtestutil.SetupSimulation(config, "leveldb-app-sim", "Simulation", simcli.FlagVerboseValue, FlagEnableSimulation)
	if skip {
...
func TestFullAppSimulation(t *testing.T) {
	...
	require.Equal(t, "lumen", app.Name())
...
func TestAppImportExport(t *testing.T) {
	...
	newDB, newDir, _, _, err := simtestutil.SetupSimulation(config, "leveldb-app-sim-2", "Simulation-2", simcli.FlagVerboseValue, FlagEnableSimulation)
	require.NoError(t, err, "simulation setup failed")

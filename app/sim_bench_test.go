...
func BenchmarkFullAppSimulation(b *testing.B) {
	...
	if !FlagEnableSimulation {
		b.Skip("skipping benchmark application simulation; pass -EnableSimulation to run")
	}

	config := simcli.NewConfigFromFlags()
	config.ChainID = SimAppChainID

	db, dir, logger, skip, err := simtestutil.SetupSimulation(config, "goleveldb-app-sim", "Simulation", simcli.FlagVerboseValue, FlagEnableSimulation)
	if err != nil {
